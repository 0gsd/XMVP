...
It would, so to speak, appear as an accident, when to a thing
...
The thing is independent, in so far as it can occur in all pos-
...
In order to know an object, I must know not its external but all
...
A spatial object must lie in infinite space. (A point in space is
...
Every statement about complexes can be analysed into a state-
...
Two objects of the same logical form are—apart from their ex-
...
The totality of existent atomic facts also determines which atom-
...
According to this view the representing relation which makes it
...
In the picture and the pictured there must be something identi-
cal in order that the one can be a picture of the other at all.

2.17

What the picture must have in common with reality in order to
...
The picture, however, cannot represent its form of representa-
...
But the picture cannot place itself outside of its form of repre-
sentation.
...
The picture contains the possibility of the state of affairs which
it represents.

2.21

The picture agrees with reality or not; it is right or wrong, true
or false.
...
An a priori true thought would be one whose possibility guar-
anteed its truth.

3.05

We could only know a priori that a thought is true if its truth
...
These elements I call "simple signs" and the proposition "com-
pletely analysed".
...
The name means the object. The object is its meaning. ("A" is
the same sign as "A".)

3.21

To the configuration of the simple signs in the propositional
...
The postulate of the possibility of the simple signs is the postu-
...
Every defined sign signifies via those signs by which it is defined,
...
What does not get expressed in the sign is shown by its appli-
cation. What the signs conceal, their application declares.

3.263

The meanings of primitive signs can be explained by elucida-
...
Only the proposition has sense; only in the context of a propo-
sition has a name meaning.

3.31

Every part of a proposition which characterizes its sense I call
...
on what, by arbitrary agreement, we mean by parts of that prop-
...
What values the propositional variable can assume is deter-
mined.
...
The determination of the values of the propositional variable
...
intransitive verb like "to go"; "identical" as an adjective; we speak
...
symbols and by not applying signs in the same way which signify
...
In order to recognize the symbol in the sign we must consider
the significant use.
...
From this observation we get a further view—into Russell's The-
ory of Types. Russell's error is shown by the fact that in drawing
up his symbolic rules he has to speak of the meaning of the signs.
...
the form $\phi(fx)$, the outer the form $\psi(\phi(fx))$. Common to
both functions is only the letter "F", which by itself signifies
nothing.

This is at once clear, if instead of "F(F(u))" we write "($\exists \phi$) :
F($\phi u$). $\phi u$=Fu".
...
In our notations there is indeed something arbitrary, but this
...
A particular method of symbolizing may be unimportant, but it
...
The sign of the complex is not arbitrarily resolved in the analysis,
in such a way that its resolution would be different in every
propositional structure.

3.4

The proposition determines a place in logical space: the exis-
...
(The logical scaffolding round the picture determines the log-
...
It is obvious that we perceive a proposition of the form aRb as
a picture. Here the sign is obviously a likeness of the signified.

4.013

And if we penetrate to the essence of this pictorial nature we
see that this is not disturbed by apparent irregularities (like the
use of # and b in the score).
...
In the proposition a state of affairs is, as it were, put together
...
The proposition is a picture of its state of affairs, only in so far
...
it by an index g, like: "f(xg)" it would not do—we should not
...
For the same reason the idealist explanation of the seeing of spa-
...
If one does not observe that propositions have a sense inde-
...
Can we not make ourselves understood by means of false propo-
...
first know under what conditions a point is called white or black;
...
black are; but to a proposition without a sense corresponds noth-
...
A property is internal if it is unthinkable that its object does
not possess it.
...
longing to it, cannot be expressed by a proposition. But it shows
...
So one cannot, e.g. say "There are objects" as one says "There
are books". Nor "There are 100 objects" or "There are $\aleph_0$ ob-
...
If we want to express in logical symbolism the general propo-
...
The simplest proposition, the elementary proposition, asserts
...
It is obvious that in the analysis of propositions we must come
...
Expressions of the form "a = b" are therefore only expedients
...
specification of all elementary propositions plus the specification,
which of them are true and which false.

4.27

With regard to the existence of n atomic facts there are $K_n = \sum_{\nu=0}^n \binom{n}{\nu}$ possibilities.
...
To these combinations correspond the same number of possibili-
...
The truth-possibilities can be presented by schemata of the fol-
...
are $\sum_{\kappa=0}^{K_n} \binom{K_n}{\kappa} = L_n$ possibilities.

4.43

Agreement with the truth-possibilities can be expressed by co-
...
false" were real objects and the arguments in ~p, etc., then
the sense of ~p would by no means be determined by Frege's
determination.)
...
For n elementary propositions there are $L_n$ possible groups of
truth-conditions.
...
The proposition shows what it says, the tautology and the con-
...
Tautology and contradiction are, however, not senseless; they
...
Suppose all elementary propositions were given me: then we
...
The elementary propositions are the truth-arguments of propo-
sitions.
...
Tractatus Logico-Philosophicus (Pages 56–60)
...
When we conclude from $p \vee q$ and $\sim p$ to $q$ the relation between the forms of the propositions "$p \vee q$" and "$\sim p$" is here concealed by the method of symbolizing. But if we write, e.g. instead of "$p \vee q$" "$p|q .|. p|q$" and instead of "$\sim p$" "$p|p$" ($p|q =$ neither $p$ nor $q$), then the inner connexion becomes obvious.
...
The method of inference is to be understood from the two propositions alone. Only they themselves can justify the inference.
...
If $T_r$ is the number of the truth-grounds of the proposition "$r$", $T_{rs}$ the number of those truth-grounds of the proposition "$s$" which are at the same time truth-grounds of "$r$", then we call the ratio $T_{rs} : T_r$ the measure of the probability which the proposition "$r$" gives to the proposition "$s$".

5.151

Suppose in a schema like that above in No. 5.101 $T_r$ is the number of the "T"'s in the proposition $r$, $T_{rs}$ the number of those "T"'s in the proposition $s$, which stand in the same columns as "T"'s of the proposition $r$; then the proposition $r$ gives to the proposition $s$ the probability $T_{rs} : T_r$.
...
Independent propositions (e.g. any two elementary propositions) give to one another the probability 1/2. 
...
Operations can vanish (e.g. denial in "~~p", $\sim\sim p = p$).
...
That $\vee$, $\supset$, etc., are not relations in the sense of right and left, etc., is obvious.
...
And it is obvious that the "$\supset$" which we define by means of "$\sim$" and "$\vee$" is identical with that by which we define "$\vee$" with the help of "$\sim$", and that this "$\vee$" is the same as the first, and so on.

5.43 

That from a fact p an infinite number of others should follow, namely $\sim\sim p$, $\sim\sim\sim\sim p$, etc., is indeed hardly to be believed, and it is no less wonderful that the infinite number of propositions of logic (of mathematics) should follow from half a dozen "primitive propositions".
...
Does "~~p" deny $\sim p$ or does it affirm p; or both?
...
This disappearance of the apparent logical constants also occurs if "$\sim(\exists x).\sim fx$" says the same as "$(x).fx$", or "$(\exists x).fx . x=a$" the same as "$fa$".
...
When we have rightly introduced the logical signs, the sense of all their combinations has been already introduced with them: therefore not only "$p \vee q$" but also "$\sim(p \vee \sim q)$", etc. etc. We should then already have introduced the effect of all possible combinations of brackets; and it would then have become clear that the proper general primitive signs are not "$p \vee q$", "$(\exists x).fx$", etc., but the most general form of their combinations.

5.461 

The apparently unimportant fact that the apparent relations like $\vee$ and $\supset$ need brackets—unlike real relations—is of great importance.
...
Hence the common rule according to which "$\sim p$", "$\sim\sim\sim p$", "$\sim p \vee \sim p$", "$\sim p . \sim p$", etc. etc. (to infinity) are constructed. And this which is common to them all mirrors denial.

5.513 

We could say: What is common to all symbols, which assert both p and q, is the proposition "$p . q$". What is common to all symbols, which assert either p or q, is the proposition "$p \vee q$".
...
Thus even in Russell's notation it is evident that "$q : p \vee \sim p$" says the same as "q"; that "$p \vee \sim p$" says nothing.
...
It must be recognized in our symbols that what is connected by "$\vee$", ".", etc., must be propositions.

And this is the case, for the symbols "p" and "q" presuppose "$\vee$", "$\sim$", etc. If the sign "p" in "$p \vee q$" does not stand for a complex sign, then by itself it cannot have sense; but then also the signs "$p \vee p$", "$p . p$", etc. which have the same sense as "p" have no sense. If, however, "$p \vee p$" has no sense, then also "$p \vee q$" can have no sense.
...
And analogously: not "$( \exists x, y).f(x, y).x=y$", but "$( \exists x).f(x,x)$"; and not "$( \exists x, y).f(x, y).\sim x=y$", but "$( \exists x, y).f(x,y)$".

(Therefore instead of Russell's "$( \exists x, y).f(x,y)$": "$( \exists x, y).f(x,y) . \vee . (\exists x).f(x,x)$".)

5.5321 

Instead of "$(x) : fx \supset x=a$" we therefore write e.g. "$(\exists x) . fx . \supset . fa : \sim(\exists x, y) . fx . fy$".

And the proposition "only one x satisfies $f( )$" reads: "$(\exists x) . fx : \sim(\exists x, y) . fx . fy$".
...
There are certain cases in which one is tempted to use expressions of the form "$a=a$" or "$p \supset p$" and of that kind. And indeed this takes place when one would like to speak of the archetype Proposition, Thing, etc. So Russell in the Principles of Mathematics has rendered the nonsense "p is a proposition" in symbols by "$p \supset p$" and has put it as hypothesis before certain propositions to show that their places for arguments could only be occupied by propositions.
...
Similarly it was proposed to express "There are no things" by "$\sim(\exists x).x=x$". But even if this were a proposition—would it not be true if indeed "There were things", but these were not identical with themselves?
...
(Figure of a Cube with vertices a and b)
...
In fact what solipsism means, is quite correct, only it cannot be said, it can only be shown.

That the world is my world, shows itself in the fact that the limits of the language (the only language I understand) indicate the boundaries of my world.
...
This says nothing else than that every proposition is the result of successive applications of the operation $N'(\bar{\xi})$ to the elementary propositions.
...
The general form of the operation $\Omega'(\bar{\eta})$ is therefore: $[\bar{\xi}, N(\bar{\xi})]'(\bar{\eta})$ (= $[\bar{\eta}, \bar{\xi}, N(\bar{\xi})]$).
...
The concept number is nothing else than that which is common to all numbers, the general form of number.
...
(Ω^ν)^μ' x = Ω^(ν x μ)' x Def.

Ω^(2 x 2)' x = (Ω^2)^2' x = (Ω^2)^(1 + 1)' x = Ω^2' Ω^2' x = Ω^(1 + 1)' Ω^(1 + 1)' x
